...
rediprune_tau.py
================
Ablation: sweep tau ∈ {0.0, 0.05, 0.1, 0.2, 0.5} in ReDiPrune.

MATH:
  tau is a relevance pre-filter threshold applied BEFORE the greedy diversity
  selection step. It controls how many tokens are eligible candidates.

  tau = 0.0 → ALL tokens are candidates (no pre-filtering):
      cand = all N tokens
      This is the most liberal setting: diversity is computed across everything.

  tau > 0  → only tokens with cos_sim(v_i, q_text) >= tau are candidates:
      cand = {i : cos_sim(Vn_i, Tn) >= tau}
      If |cand| < K, fall back to topK by relevance to ensure K tokens are chosen.

  Effect of tau:
    - Small tau (e.g. 0.05): loose gate, most tokens pass, close to tau=0
    - Medium tau (e.g. 0.1): balanced — relevant tokens only
    - Large tau (e.g. 0.5): very strict gate, only highly relevant tokens compete
      for diversity; selection is more focused but may miss diverse coverage

  tau interacts with alpha: with small alpha (diversity-heavy), tau has less effect
  since diversity dominates once candidates are selected. With large alpha, tau
  more aggressively pre-filters to only relevance-aligned frames before the
  diversity scoring ranks within that subset.

  Pre-filter implementation:
    When tau > 0 AND K < P // 2:
      Use topK(rel, max(K*3, K+1)) as cand  (fast approximate gate)
    When tau > 0 AND K >= P // 2:
      Use mask = rel >= tau; if |mask| >= K use mask, else topK fallback

Fixed: alpha=0.5, keep_ratio=0.5, prune_mode=frame

Usage:
  # Run all tau values sequentially:
  python rediprune_tau.py --metadata metadata.json --videos /data/videos

  # Run a single tau value (for parallel GPU execution):
  python rediprune_tau.py --metadata metadata.json --videos /data/videos --value 0.1
"""
...
TAU_VALUES = [0.0, 0.05, 0.1, 0.2, 0.5]
...
def rediprune_select(visual_tokens, text_query, keep_ratio, alpha, tau):
    ...
    tau: relevance pre-filter threshold.
      tau=0.0 → no pre-filtering (all P tokens are candidates)
      tau>0   → only tokens with rel_i >= tau are candidates (fallback to topK if < K pass)
    """
    ...
    # Pre-filter candidates by tau
    if tau > 0:
        if K < P // 2:
            # Approx gate: take top 3K candidates by relevance
            ...
            if mask.sum() >= K:
                ...
                # Fallback: topK by relevance if threshold too strict
...
def run_inference(model, processor, video_path, dataset, question, choices,
    ...
    # Frame-level pruning with specified tau
    pruned_videos = []
    for vid in videos:
...
def run_tau(tau, args, model, processor, meta, out_base):
    slug = f"tau_{tau:.2f}".replace(".", "p")
    ...
    print(f"\n{'='*60}")
    print(f"tau={tau:.2f} | alpha={FIXED_ALPHA} | keep_ratio={FIXED_KEEP_RATIO}")
    if tau == 0.0:
        print("  Pre-filter: DISABLED (all tokens are candidates)")
    else:
        print(f"  Pre-filter: cos_sim(v_i, q_text) >= {tau:.2f}  (fallback to topK if < K pass)")
            ...
            for q in entry.get("questions", []):
                ...
                try:
                    pred, reasoning, orig_nf, used_nf, prefill_ms = run_inference(
                        ...
                        FIXED_KEEP_RATIO, FIXED_ALPHA, tau,
                ...
                rec = {
                    ...
                    "method": "rediprune_tau",
                    "config": {"alpha": FIXED_ALPHA, "keep_ratio": FIXED_KEEP_RATIO, "tau": tau, "prune_mode": FIXED_PRUNE_MODE},
                    "ablation_param": "tau", "ablation_value": tau,
                }
    ...
    print(f"  tau={tau:.2f}: {correct}/{total} = {acc:.2%}")
    return {"tau": tau, "accuracy": round(acc, 4), "correct": correct, "total": total}


# ── Main ─────────────────────────────────────────────────────────────────────

def main():
    parser = argparse.ArgumentParser(description="ReDiPrune tau (relevance threshold) ablation")
    ...
    parser.add_argument("--output_base", default=None,
                        help="Base output dir. Defaults to ablation_outputs/rediprune_tau/")
    parser.add_argument("--fps", type=float, default=DEFAULT_FPS)
    parser.add_argument("--max_pixels", type=int, default=DEFAULT_MAX_PIXELS)
    parser.add_argument("--no_audio", action="store_true")
    parser.add_argument("--value", type=float, default=None,
                        help="Run a single tau value (for parallel GPU execution). "
                             "If omitted, loops over all: " + str(TAU_VALUES))
    args = parser.parse_args()

    out_base = args.output_base or os.path.join(
        os.path.dirname(_REPO_ROOT), "ablation_outputs", "rediprune_tau"
    )
    ...
    values = [args.value] if args.value is not None else TAU_VALUES
    summary_results = []

    for tau in values:
        res = run_tau(tau, args, model, processor, meta, out_base)
        summary_results.append(res)

    summary = {
        "ablation": "rediprune_tau",
        "fixed": {"alpha": FIXED_ALPHA, "keep_ratio": FIXED_KEEP_RATIO, "prune_mode": FIXED_PRUNE_MODE},
    ...
    for r in summary_results:
        print(f"  tau={r['tau']:.2f}: accuracy={r['accuracy']:.4f} ({r['correct']}/{r['total']})")
