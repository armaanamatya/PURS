"""
omnizip_rho_video.py
====================
ABLATION: Sweep rho_video ∈ {0.3, 0.45, 0.6, 0.75, 0.9}

rho_video (merging_ratio_v) is the GLOBAL TARGET video merging ratio fed into
the omnizip() function as `merging_ratio_v`. The per-group ratios are audio-
guided (ranging 0.35–0.75) and then normalised so their mean equals rho_video.

  Higher rho_video → more video groups targeted for merging → fewer video tokens
  Lower  rho_video → more video tokens kept

Default OmniZip paper setting: rho_video=0.6 (i.e., keep ~40% of video groups
on average, with cross-modal audio guidance modulating per-group ratios).

All other params fixed at defaults:
  rho_audio=0.3, g=3, contextual_ratio=0.05

Usage (run one value at a time for parallelism across GPUs):
    python omnizip_rho_video.py --value 0.6 \\
        --metadata /data/armaan/purs/metadata.json \\
        --videos   /data/armaan/purs/videos \\
        --output_root /data/armaan/purs/ablation_outputs

Or loop all values (sequential, single GPU):
    python omnizip_rho_video.py --all_values \\
        --metadata /data/armaan/purs/metadata.json \\
        --videos   /data/armaan/purs/videos
"""
...
_REPO_ROOT   = os.path.dirname(_SCRIPT_DIR)          # .../PURS/
...
# ── Constants ──────────────────────────────────────────────────────────────────
ABLATION_NAME = "omnizip_rho_video"

SWEEP_VALUES = [0.3, 0.45, 0.6, 0.75, 0.9]
SLUG_MAP     = {0.3: "0p30", 0.45: "0p45", 0.6: "0p60", 0.75: "0p75", 0.9: "0p90"}
...
# Fixed params for this ablation
FIXED_RHO_AUDIO       = 0.3
FIXED_G               = 3
...
# ── Model loading ──────────────────────────────────────────────────────────────
def load_model(model_path, rho_video, rho_audio, g, contextual_ratio):
    """Load Qwen2.5-Omni + OmniZip with given compression config."""
    ...
    print(f"  Model loaded. VRAM allocated: {alloc_gb:.2f} GB")
...
# ── Per-value runner ───────────────────────────────────────────────────────────
def run_value(rho_video, args, out_dir):
    ...
    tee = Tee(console_path, label=f"rho_video={rho_video}")
    stee = StderrTee(stderr_path)
    sys.stdout, sys.stderr = tee, stee

    try:
        model, processor, model_alloc_gb = load_model(
            args.model, rho_video, FIXED_RHO_AUDIO, FIXED_G, FIXED_CONTEXTUAL_RATIO
        )
            ...
            for entry in runnable:
                ...
                for q in entry["questions"]:
                    ...
                    result = {
                        ...
                        "config": {
                            "rho_video": rho_video, "rho_audio": FIXED_RHO_AUDIO,
                            "g": FIXED_G, "contextual_ratio": FIXED_CONTEXTUAL_RATIO,
                        },
                        "ablation_value": rho_video,
                        "status": status,
                    }
                    if err_msg:
        ...
        summary = {
            "ablation": ABLATION_NAME, "value": rho_video,
            "config": {"rho_video": rho_video, "rho_audio": FIXED_RHO_AUDIO,
                       "g": FIXED_G, "contextual_ratio": FIXED_CONTEXTUAL_RATIO},
            "correct": correct, "total": total, "accuracy": round(acc, 4),
            "skipped": skipped,
        }
        Path(summary_path).write_text(json.dumps(summary, indent=2))
        print(f"\n  rho_video={rho_video}: {correct}/{total} = {acc:.2%}")
        return summary
...
def main():
    ...
    parser.add_argument("--value",    type=float, default=None,
                        help="Single rho_video value to run (e.g. 0.6). Omit for --all_values.")
    parser.add_argument("--all_values", action="store_true",
                        help="Run all sweep values sequentially.")
    ...
    if args.value is None and not args.all_values:
        parser.error("Specify --value <float> or --all_values")
    ...
    all_summaries = []
    for v in values:
        slug = SLUG_MAP.get(v, str(v).replace(".", "p"))
        out_dir = os.path.join(sweep_dir, f"rho_{slug}")
        print(f"\n{'='*60}")
        print(f"[{ABLATION_NAME}] rho_video={v}  → {out_dir}")
        print(f"{'='*60}")
    ...
    for s in all_summaries:
        print(f"  rho_video={s['value']}: {s['correct']}/{s['total']} = {s['accuracy']:.2%}")
