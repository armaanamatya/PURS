"""
omnizip_contextual.py
=====================
ABLATION: Sweep contextual_ratio ∈ {0.0, 0.025, 0.05, 0.1, 0.2}

contextual_ratio controls how many additional audio tokens are uniformly sampled
as "contextual anchors" BEYOND the top-dominant attention-selected tokens.

Inside omnizip_audio_attn():
    dominant_num  = round((1 - rho_audio) * N)   ← attention-selected (kept)
    contextual_num = round(contextual_ratio * N)   ← uniform anchors (additional kept)

These anchors serve as merge targets for the remaining (dropped) audio tokens.
Without anchors (contextual_ratio=0.0), only dominant tokens are kept — pure
attention-guided selection, no uniform spatial coverage.

Effect:
  contextual_ratio=0.0  → pure attention-guided, no safety net; more aggressive
  contextual_ratio=0.2  → 20% additional uniform coverage; less speculative pruning

The anchors also serve as targets for the g-nearest-neighbor merging step.
A higher contextual_ratio → more merge targets → more audio info preserved.

Fixed params: rho_video=0.6, rho_audio=0.3, g=3

Usage:
    python omnizip_contextual.py --value 0.05 \\
        --metadata /data/armaan/purs/metadata.json \\
        --videos   /data/armaan/purs/videos

    python omnizip_contextual.py --all_values ...
"""
...
# ── Constants ──────────────────────────────────────────────────────────────────
ABLATION_NAME = "omnizip_contextual"

SWEEP_VALUES = [0.0, 0.025, 0.05, 0.1, 0.2]
SLUG_MAP     = {
    0.0:   "0p000",
    0.025: "0p025",
    0.05:  "0p050",
...
# ── Model loading ──────────────────────────────────────────────────────────────
def load_model(model_path, contextual_ratio):
    print(f"[{ABLATION_NAME}] Loading model: {model_path}")
    print(f"  contextual_ratio={contextual_ratio}  rho_video={FIXED_RHO_VIDEO}  "
          f"rho_audio={FIXED_RHO_AUDIO}  g={FIXED_G}")
...
# ── Per-value runner ───────────────────────────────────────────────────────────
def run_value(contextual_ratio, args, out_dir):
    ...
    tee  = Tee(console_path, label=f"contextual_ratio={contextual_ratio}")
    stee = StderrTee(stderr_path)
    sys.stdout, sys.stderr = tee, stee

    try:
        model, processor, model_alloc_gb = load_model(args.model, contextual_ratio)
                ...
                for q in entry["questions"]:
                    ...
                    result = {
                        ...
                        "config": {
                            ...
                            "g": FIXED_G, "contextual_ratio": contextual_ratio,
                        },
                        "ablation_value": contextual_ratio,
                        "status": status,
                    }
                    if err_msg:
        ...
        summary = {
            "ablation": ABLATION_NAME, "value": contextual_ratio,
        ...
        print(f"\n  contextual_ratio={contextual_ratio}: {correct}/{total} = {acc:.2%}")
        return summary
...
def main():
    ...
    parser.add_argument("--value",    type=float, default=None,
                        help="Single contextual_ratio value (e.g. 0.05).")
    ...
    for v in values:
        ...
        out_dir = os.path.join(sweep_dir, f"ratio_{slug}")
        print(f"\n{'='*60}")
        print(f"[{ABLATION_NAME}] contextual_ratio={v}  → {out_dir}")
        print(f"{'='*60}")
    ...
    for s in all_summaries:
        print(f"  contextual_ratio={s['value']}: {s['correct']}/{s['total']} = {s['accuracy']:.2%}")
