...
omnizip_sanity.py
=================
ABLATION: Sanity check — OmniZip with near-full token retention vs. default.

TWO CONDITIONS:

  1. "near_full" — rho_video=0.99, rho_audio=0.99
     Nearly all tokens are kept; effectively no compression.
     Expected accuracy ≈ uncompressed baseline (Qwen2.5-Omni-7B full-KV).
     Any accuracy gap vs. the true baseline reveals compression overhead.

     Note: rho_video=1.0 is avoided because:
       - dominant_num = round((1 - 1.0) * N) = 0 → no tokens selected via attention
       - All tokens fall into the "remaining" pool; contextual_ratio still keeps some
       - This edge case is tested separately as rho_video=0.99 which is safer

  2. "default" — rho_video=0.6, rho_audio=0.3 (standard OmniZip paper config)
     Runs the normal OmniZip configuration as a within-script reference point.

Together these two conditions let you confirm:
  a) The eval harness is working correctly (near_full ≈ baseline)
  b) Default OmniZip is performing as expected relative to near-full
  c) The accuracy delta is attributable to compression, not other factors

Output: ablation_outputs/omnizip_sanity/near_full/ and default/

Usage:
    python omnizip_sanity.py --condition near_full \\
        --metadata /data/armaan/purs/metadata.json \\
        --videos   /data/armaan/purs/videos

    python omnizip_sanity.py --condition default ...

    python omnizip_sanity.py --all_conditions ...
"""
...
# ── Constants ──────────────────────────────────────────────────────────────────
ABLATION_NAME = "omnizip_sanity"

# (condition_name, rho_video, rho_audio, description)
CONDITION_CONFIGS = {
    "near_full": {
        "rho_video": 0.99,
        "rho_audio": 0.99,
        ...
        "description": "Near-full token retention (rho_video=0.99, rho_audio=0.99) — sanity upper bound",
    },
    "default": {
        "rho_video": 0.6,
        "rho_audio": 0.3,
        "g": 3,
        "contextual_ratio": 0.05,
        "description": "Standard OmniZip paper config (rho_video=0.6, rho_audio=0.3)",
    },
}
...
def load_model(model_path, cfg):
    ...
    model.thinker.omnizip_config = {
        "rho_audio":        cfg["rho_audio"],
        "rho_video":        cfg["rho_video"],
        "g":                cfg["g"],
        "contextual_ratio": cfg["contextual_ratio"],
    }
...
def run_condition(condition, args, out_dir):
    assert condition in CONDITION_CONFIGS, f"Unknown condition: {condition}"
    ...
    tee  = Tee(console_path, label=f"condition={condition}: {cfg['description']}")
    stee = StderrTee(stderr_path)
    sys.stdout, sys.stderr = tee, stee

    try:
        model, processor, model_alloc_gb = load_model(args.model, cfg)
        ...
        print(f"  Condition: {condition} — {cfg['description']}")
        ...
        summary = {
            ...
            "config": {
                "rho_video": cfg["rho_video"], "rho_audio": cfg["rho_audio"],
                "g": cfg["g"], "contextual_ratio": cfg["contextual_ratio"],
            },
...
def main():
    ...
    parser.add_argument("--condition", choices=CONDITIONS, default=None,
                        help="Which condition: 'near_full' or 'default'.")
    ...
    if args.condition is None and not args.all_conditions:
        parser.error("Specify --condition {near_full,default} or --all_conditions")
    ...
    for cond in conds:
        ...
        print(f"  {CONDITION_CONFIGS[cond]['description']}")
        print(f"{'='*60}")
    ...
    for s in all_summaries:
        print(f"  {s['condition']:12s}  acc={s['accuracy']:.2%}  ({s['correct']}/{s['total']})")
        print(f"    config: rho_video={s['config']['rho_video']}  rho_audio={s['config']['rho_audio']}")

    if len(all_summaries) == 2:
        near = next((s for s in all_summaries if s["condition"] == "near_full"), None)
        dflt = next((s for s in all_summaries if s["condition"] == "default"), None)
        if near and dflt:
            gap = near["accuracy"] - dflt["accuracy"]
            print(f"\n  Compression gap (near_full - default): {gap:+.2%}")
            if abs(gap) < 0.02:
                print("  → Negligible gap — compression has minimal accuracy impact.")
            elif gap > 0.02:
                print("  → Positive gap — compression reduces accuracy; expect this.")
            else:
                print("  → Negative gap — default outperforms near_full (unexpected; check eval).")
