"""
noaudio_all_methods.py
Ablation: All 5 methods run with --no_audio (video+text only, no audio input).

Purpose: measures how much the AUDIO MODALITY (as model input) contributes to each method.
- baseline:  full model, no audio input
- omnizip:   rho_v=0.6, rho_a=0.3, g=3, contextual_ratio=0.05, no audio input
- rediprune: keep_ratio=0.5, alpha=0.5, tau=0.1, no audio input
- divprune:  keep_ratio=0.5 (diversity selection), no audio input
- mixkv:     budget=256, snapkv, no audio input

Audio is NEVER loaded even if check_video_has_audio() returns True — the whole
point of this ablation is to ablate the audio modality.

Output: ablation_outputs/noaudio_methods/{baseline,omnizip,rediprune,divprune,mixkv}/
  Each: results.jsonl, vram_log.jsonl, console.log, stderr.log, run_summary.json
Final: ablation_outputs/noaudio_methods/comparison_summary.json

Usage:
  python noaudio_all_methods.py --metadata /data/armaan/purs/metadata.json \
      --videos /data/armaan/purs/videos
  python noaudio_all_methods.py ... --condition omnizip
"""
...
# Method configurations — all run with use_audio=False
CONDITIONS = {
    "baseline": {
        "method": "baseline",
        "use_omnizip": False, "rho_video": None, "rho_audio": None,
        "g": None, "contextual_ratio": None,
    ...
    "omnizip": {
        "method": "omnizip",
        "use_omnizip": True, "rho_video": 0.6, "rho_audio": 0.3,
        "g": 3, "contextual_ratio": 0.05,
        "use_rediprune": False, "keep_ratio": None, "alpha": None, "tau": None,
        "use_divprune": False,
        "use_mixkv": False, "budget": None, "window_size": None, "select_method": None,
    },
    ...
    "divprune": {
        ...
        "use_omnizip": False, "rho_video": None, "rho_audio": None,
        "g": None, "contextual_ratio": None,
        "use_rediprune": False, "keep_ratio": 0.5, "alpha": None, "tau": None,
        "use_divprune": True,
        "use_mixkv": False, "budget": None, "window_size": None, "select_method": None,
    },
    "mixkv": {
...
# ── DivPrune ──────────────────────────────────────────────────────────────────
def divprune_select(features, keep_count):
    ...
    dist = 1.0 - torch.mm(normed, normed.t())  # (N,N) cosine distance
    sel = torch.empty(keep_count, dtype=torch.long, device=features.device)
    # First: most isolated token (max of second-smallest distance)
    topk = torch.topk(dist, k=2, dim=0, largest=False).values
    sel[0] = topk[1, :].argmax()
    for i in range(1, keep_count):
        min_dists = dist[sel[:i], :].min(dim=0).values
        sel[i] = min_dists.argmax()
    return sel
...
def run_inference(model, processor, video_path, dataset, question, choices,
                  run_cfg, cond_cfg):
    # no_audio ablation: audio is NEVER used regardless of stream presence
    use_audio = False
    ...
    # Frame-level selection (ReDiPrune or DivPrune)
    if cond_cfg["use_rediprune"] or cond_cfg["use_divprune"]:
        vid_tensor = videos[0]
        if vid_tensor.dim() == 4:
        ...
        keep_count = max(1, int(round(orig_frames * cond_cfg["keep_ratio"])))
        ...
        if cond_cfg["use_rediprune"]:
            ...
            idx = divprune_select(features, keep_count)
...
# ── Run one condition ─────────────────────────────────────────────────────────
def run_condition(cond_name, cond_cfg, meta, videos_dir, output_base, run_cfg):
    ...
    print(f"\nCondition: {cond_name} (no_audio=True always)")
    ...
    with open(results_path, "w") as rf, open(vram_path, "w") as vf:
        for entry in runnable:
            ...
            for q in entry["questions"]:
                ...
                rf.write(json.dumps({
                    "condition": cond_name, "method": cond_cfg["method"],
                    "no_audio": True,
    ...
    summary = {
        "condition": cond_name, "method": cond_cfg["method"],
        "no_audio": True, "correct": correct, "total": total,
...
def main():
    parser = argparse.ArgumentParser(
        description="Ablation: All methods with no audio input (video+text only)")
    ...
    parser.add_argument("--output_base",
                        default="/data/armaan/purs/ablation_outputs/noaudio_methods")
    parser.add_argument("--condition",   default=None, choices=list(CONDITIONS.keys()),
                        help="Run a single method condition (default: all 5)")
    ...
    print("NOTE: Audio is disabled for ALL conditions in this ablation.")
    ...
    for cond_name, cond_cfg in to_run.items():
        summary = run_condition(cond_name, cond_cfg, meta, args.videos,
                                args.output_base, run_cfg)
        all_summaries[cond_name] = summary

    cmp_path = os.path.join(args.output_base, "comparison_summary.json")
    with open(cmp_path, "w") as f:
        json.dump(all_summaries, f, indent=2)
    print(f"\nComparison summary (no-audio): {cmp_path}")
    print(f"{'Method':<15}  {'Accuracy':>10}  {'Correct/Total':>15}")
    print("-" * 45)
    for name, s in all_summaries.items():
        print(f"  {name:<13}  {s['accuracy']:>10.4f}  "
