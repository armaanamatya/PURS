"""
omnizip_rho_audio.py
====================
ABLATION: Sweep rho_audio ∈ {0.1, 0.2, 0.3, 0.5, 0.7}

rho_audio (merging_ratio_audio) is passed to omnizip_audio_attn() as `merging_ratio`.
Inside that function:
    dominant_num = round((1 - rho_audio) * N_audio)
This is the number of audio tokens KEPT via attention-importance top-k.

Effect on audio-guided video compression:
  - Lower rho_audio → more audio tokens kept → stronger audio-guidance signal
    for per-group video merging ratios
  - Higher rho_audio → fewer audio tokens kept → weaker audio-guidance signal,
    more tokens pushed into contextual anchors / merged away

Also note that the audio mask determines audio_group_retention, which then
modulates per-group video merging ratios:
    video_merging_ratio[i] = 0.75 + (0.35 - 0.75) * audio_group_retention[i]

rho_audio=0.1 → keep 90% of audio (very mild compression, maximum signal)
rho_audio=0.7 → keep 30% of audio (aggressive, weaker cross-modal signal)

Fixed params: rho_video=0.6, g=3, contextual_ratio=0.05

Usage:
    python omnizip_rho_audio.py --value 0.3 \\
        --metadata /data/armaan/purs/metadata.json \\
        --videos   /data/armaan/purs/videos

    python omnizip_rho_audio.py --all_values ...
"""
...
# ── Constants ──────────────────────────────────────────────────────────────────
ABLATION_NAME = "omnizip_rho_audio"

SWEEP_VALUES = [0.1, 0.2, 0.3, 0.5, 0.7]
SLUG_MAP     = {0.1: "0p10", 0.2: "0p20", 0.3: "0p30", 0.5: "0p50", 0.7: "0p70"}
...
# ── Model loading ──────────────────────────────────────────────────────────────
def load_model(model_path, rho_audio, rho_video, g, contextual_ratio):
...
# ── Per-value runner ───────────────────────────────────────────────────────────
def run_value(rho_audio, args, out_dir):
    ...
    tee  = Tee(console_path, label=f"rho_audio={rho_audio}")
    stee = StderrTee(stderr_path)
    sys.stdout, sys.stderr = tee, stee

    try:
        model, processor, model_alloc_gb = load_model(
            args.model, rho_audio, FIXED_RHO_VIDEO, FIXED_G, FIXED_CONTEXTUAL_RATIO
        )
            ...
            for entry in runnable:
                ...
                for q in entry["questions"]:
                    ...
                    result = {
                        ...
                        "config": {
                            "rho_video": FIXED_RHO_VIDEO, "rho_audio": rho_audio,
                            "g": FIXED_G, "contextual_ratio": FIXED_CONTEXTUAL_RATIO,
                        },
                        "ablation_value": rho_audio,
                        "status": status,
                    }
                    if err_msg:
        ...
        summary = {
            "ablation": ABLATION_NAME, "value": rho_audio,
            "config": {"rho_video": FIXED_RHO_VIDEO, "rho_audio": rho_audio,
                       "g": FIXED_G, "contextual_ratio": FIXED_CONTEXTUAL_RATIO},
            "correct": correct, "total": total, "accuracy": round(acc, 4),
            "skipped": skipped,
        }
        Path(summary_path).write_text(json.dumps(summary, indent=2))
        print(f"\n  rho_audio={rho_audio}: {correct}/{total} = {acc:.2%}")
        return summary
...
def main():
    ...
    parser.add_argument("--value",    type=float, default=None,
                        help="Single rho_audio value to run (e.g. 0.3).")
    ...
    for v in values:
        ...
        print(f"\n{'='*60}")
        print(f"[{ABLATION_NAME}] rho_audio={v}  → {out_dir}")
        print(f"{'='*60}")
    ...
    for s in all_summaries:
        print(f"  rho_audio={s['value']}: {s['correct']}/{s['total']} = {s['accuracy']:.2%}")
