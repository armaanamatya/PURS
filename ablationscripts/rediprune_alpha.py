...
rediprune_alpha.py
==================
Ablation: sweep alpha ∈ {0.0, 0.25, 0.5, 0.75, 1.0} in ReDiPrune.

MATH:
  score_i = min_{j∈S} (1 - cos_sim(v_i, v_j))   +   alpha * cos_sim(v_i, q_text)
              ↑ diversity term (DivPrune)                ↑ text-relevance term (ReDiPrune)

  alpha = 0.0 → pure DivPrune: token selection ignores the question entirely,
                choosing only the most spatially diverse frames.
  alpha = 1.0 → max relevance weight: ties are broken almost entirely by how
                similar each frame feature is to the text query embedding.
  alpha ∈ (0, 1) → balanced trade-off (paper default: 0.5).

This is the CORE ReDiPrune ablation: demonstrates that adding text relevance
to pure diversity selection yields measurable accuracy gains.

Fixed: keep_ratio=0.5, tau=0.1, prune_mode=frame

Usage:
  # Run all alpha values sequentially (single GPU):
  python rediprune_alpha.py --metadata metadata.json --videos /data/videos

  # Run a single alpha value (for parallel GPU execution):
  python rediprune_alpha.py --metadata metadata.json --videos /data/videos --value 0.5
"""
...
def run_inference(model, processor, video_path, dataset, question, choices,
    ...
    # Measure prefill (generate 1 token)
...
def run_alpha(alpha, args, model, processor, meta, out_base):
    slug = f"alpha_{alpha:.2f}".replace(".", "p")
    ...
    print(f"\n{'='*60}")
    print(f"alpha={alpha:.2f} | keep_ratio={FIXED_KEEP_RATIO} | tau={FIXED_TAU}")
            ...
            for q in entry.get("questions", []):
                ...
                try:
                    pred, reasoning, orig_nf, used_nf, prefill_ms = run_inference(
                        ...
                        FIXED_KEEP_RATIO, alpha, FIXED_TAU,
                ...
                rec = {
                    ...
                    "method": "rediprune_alpha",
                    "config": {"alpha": alpha, "keep_ratio": FIXED_KEEP_RATIO, "tau": FIXED_TAU, "prune_mode": FIXED_PRUNE_MODE},
                    "ablation_param": "alpha", "ablation_value": alpha,
                }
    ...
    print(f"  alpha={alpha:.2f}: {correct}/{total} = {acc:.2%}")
    return {"alpha": alpha, "accuracy": round(acc, 4), "correct": correct, "total": total}


# ── Main ─────────────────────────────────────────────────────────────────────

def main():
    parser = argparse.ArgumentParser(description="ReDiPrune alpha ablation")
    ...
    parser.add_argument("--output_base", default=None,
                        help="Base output dir. Defaults to ablation_outputs/rediprune_alpha/")
    parser.add_argument("--fps", type=float, default=DEFAULT_FPS)
    parser.add_argument("--max_pixels", type=int, default=DEFAULT_MAX_PIXELS)
    parser.add_argument("--no_audio", action="store_true")
    parser.add_argument("--value", type=float, default=None,
                        help="Run a single alpha value (for parallel GPU execution). "
                             "If omitted, loops over all: " + str(ALPHA_VALUES))
    args = parser.parse_args()

    out_base = args.output_base or os.path.join(
        os.path.dirname(_REPO_ROOT), "ablation_outputs", "rediprune_alpha"
    )
    ...
    values = [args.value] if args.value is not None else ALPHA_VALUES
    summary_results = []

    for alpha in values:
        res = run_alpha(alpha, args, model, processor, meta, out_base)
        summary_results.append(res)

    summary = {"ablation": "rediprune_alpha", "fixed": {"keep_ratio": FIXED_KEEP_RATIO, "tau": FIXED_TAU, "prune_mode": FIXED_PRUNE_MODE}, "results": summary_results}
    ...
    for r in summary_results:
        print(f"  alpha={r['alpha']:.2f}: accuracy={r['accuracy']:.4f} ({r['correct']}/{r['total']})")
